...
export const BookmarkView: React.FC<BookmarkViewProps> = ({
  ...
  const dnd = useBookmarkDragAndDrop();
  ...
  const bookmarkedItemsList = useMemo(() => {
    ...
    const processItemId = (id: number) => {
      ...
      itemInfoMap.set(id, { id, name: getLocalizedText(itemInfo, lang), type, isTimed, nodeInfo: nodeRef, hidden: hiddenItemIds.has(id) });
    };
  ...
  const groupDisplaySections = useMemo(
    () => bookmarkGroups.map(group => ({ group, displayItems: getDisplayItems(group.itemIds) })),
    // eslint-disable-next-line react-hooks/exhaustive-deps
    [bookmarkGroups, bookmarkedItemsList, completedItems, hideCompleted],
  );

  const ungroupedDisplayItems = useMemo(
    () => getDisplayItems(ungroupedBookmarkedItemIds),
    // eslint-disable-next-line react-hooks/exhaustive-deps
  ...
  const hasAnyVisibleBookmarks =
    groupDisplaySections.some(s => s.displayItems.length > 0) ||
  ...
  const sharedCardProps = useMemo(() => ({
    ...
    canDrag: !!moveBookmarkedItem,
    setDragItemId: dnd.setDragItemId,
    setDropContainerId: dnd.setDropContainerId,
    setDropBeforeItemId: dnd.setDropBeforeItemId,
    clearDragState: dnd.clearDragState,
    maybeAutoScrollOnDrag: dnd.maybeAutoScrollOnDrag,
    onDropItem: moveBookmarkedItem,
    bookmarkGroups,
    onMoveToGroup: moveBookmarkedItem ? (itemId: number, groupId: string | null) => moveBookmarkedItem(itemId, groupId) : undefined,
    showGroupSelector: !!moveBookmarkedItem && (bookmarkGroups.length > 0),
    data,
    toggleComplete,
    toggleBookmark,
  ...
  // ---------------------------------------------------------------------------
  const renderGroupSection = (group: BookmarkGroup, displayItems: ItemInfo[]) => {
    const timedItemIdsInGroup = group.itemIds.filter(itemId => bookmarkedItemsList.get(itemId)?.isTimed === true);
    const trackedTimedCount = timedItemIdsInGroup.reduce((n, id) => n + (trackedItemSet.has(id) ? 1 : 0), 0);
        ...
        <GroupHeader
          ...
          inlineContent={hasTimedItems && setTrackedItems ? (
            <>
              ...
              <button type="button" onClick={() => setTrackedItems(timedItemIdsInGroup, true)}
              ...
              <button type="button" onClick={() => setTrackedItems(timedItemIdsInGroup, false)}
          ...
          onCollapse={updateGroup ? (groupId, collapsed) => updateGroup(groupId, { collapsed }) : undefined}
          onRename={updateGroup ? (groupId, newName) => updateGroup(groupId, { name: newName }) : undefined}
        ...
        {!group.collapsed && (
          <div
            className={`grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 p-4 border-b border-slate-200 dark:border-slate-600 transition-colors ${
              dnd.dragItemId && dnd.dropContainerId === group.id && !dnd.dropBeforeItemId
                ? 'bg-blue-50 dark:bg-blue-900/20' : 'bg-white dark:bg-slate-800'
            }`}
            onDragOver={(e) => {
              if (!dnd.dragItemId || !moveBookmarkedItem) return;
              e.preventDefault();
              dnd.maybeAutoScrollOnDrag(e.clientY);
              e.dataTransfer.dropEffect = 'move';
              dnd.setDropContainerId(group.id);
              dnd.setDropBeforeItemId(null);
            }}
            onDrop={(e) => {
              if (!dnd.dragItemId || !moveBookmarkedItem) return;
              e.preventDefault();
              moveBookmarkedItem(dnd.dragItemId, group.id);
              dnd.clearDragState();
            }}
            onDragLeave={(e) => {
              if (!e.currentTarget.contains(e.relatedTarget as Node) && dnd.dropContainerId === group.id) {
                dnd.setDropContainerId(null);
                dnd.setDropBeforeItemId(null);
              }
            }}
          >
            {displayItems.length > 0 ? displayItems.map(item => (
              <BookmarkItemCard
                ...
                containerDisplayId={group.id}
                itemListInContainer={group.itemIds}
  ...
  // ---------------------------------------------------------------------------
  // Ungrouped item grid (shared between timed and regular sections)
  // ---------------------------------------------------------------------------
  const renderUngroupedGrid = (items: ItemInfo[], collapsed: boolean, sectionId: string) => {
    if (collapsed) return null;
    return (
      <div
        className={`grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 p-4 border-b border-slate-200 dark:border-slate-600 transition-colors ${
          dnd.dragItemId && dnd.dropContainerId === sectionId && !dnd.dropBeforeItemId
            ? 'bg-blue-50 dark:bg-blue-900/20' : 'bg-white dark:bg-slate-800'
        }`}
        onDragOver={(e) => {
          if (!dnd.dragItemId || !moveBookmarkedItem) return;
          ...
          dnd.maybeAutoScrollOnDrag(e.clientY);
          e.dataTransfer.dropEffect = 'move';
          dnd.setDropContainerId(sectionId);
          dnd.setDropBeforeItemId(null);
        }}
        onDrop={(e) => {
          if (!dnd.dragItemId || !moveBookmarkedItem) return;
          e.preventDefault();
          moveBookmarkedItem(dnd.dragItemId, null);
          dnd.clearDragState();
        }}
        onDragLeave={(e) => {
          if (!e.currentTarget.contains(e.relatedTarget as Node) && dnd.dropContainerId === sectionId) {
            dnd.setDropContainerId(null);
            dnd.setDropBeforeItemId(null);
          }
        }}
      >
        {items.map(item => (
          <BookmarkItemCard
            ...
            item={item}
            currentGroupId={null}
            containerDisplayId={sectionId}
            itemListInContainer={ungroupedBookmarkedItemIds}
            isCompleted={completedItems.has(item.id)}
            isAlarmTracked={trackedItemSet.has(item.id)}
          />
        ))}
  ...
  // Empty state
  // ---------------------------------------------------------------------------
  if (bookmarkedItems.size === 0 && bookmarkGroups.length === 0) {
    return (
  ...
  return (
    <div
      ...
      onDragOverCapture={(e) => { if (dnd.dragItemId && moveBookmarkedItem) dnd.maybeAutoScrollOnDrag(e.clientY); }}
    >
      {/* Group overview card */}
      <div className="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-slate-200 dark:border-slate-700 overflow-hidden">
        <div className="px-6 py-4 bg-violet-50 dark:bg-violet-900/10 border-b border-violet-100 dark:border-violet-900/30 flex flex-col items-start gap-3 sm:flex-row sm:items-center sm:justify-between">
          ...
          {createGroup && (
            <div className="flex w-full flex-col items-start gap-2 sm:w-auto sm:flex-row sm:flex-wrap sm:items-center sm:justify-end">
              ...
              <button
                type="button"
                onClick={() => { if (bookmarkGroups.length < maxBookmarkGroups) createGroup(nextDefaultGroupName); }}
        ...
        <div className="divide-y divide-slate-200 dark:divide-slate-600">
          {groupDisplaySections.map(({ group, displayItems }) => renderGroupSection(group, displayItems))}
        </div>
      </div>
        ...
        <div className="space-y-8">
          {timedUngroupedItems.length > 0 && (
            <div className="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-slate-200 dark:border-slate-700 overflow-hidden">
              ...
              <div className="divide-y divide-slate-200 dark:divide-slate-600">
                <GroupHeader
                  group={{ id: '__ungrouped_timed__', name: i18n.pages.gathering_log.ungrouped, itemIds: timedUngroupedItems.map(i => i.id), collapsed: isTimedUngroupedCollapsed, createdAt: '', updatedAt: '' }}
                  itemCount={timedUngroupedItems.length}
                  inlineContent={setTrackedItems ? (() => {
                    const timedIds = timedUngroupedItems.map(i => i.id);
                    const trackedCount = timedIds.filter(id => trackedItemSet.has(id)).length;
                    return (
                      <>
                        <span className="text-xs font-medium text-slate-600 dark:text-slate-300 whitespace-nowrap">
                          {i18n.pages.gathering_log.bookmark_alarm_status.replace('{tracked}', String(trackedCount)).replace('{total}', String(timedIds.length))}
                        </span>
                        <button type="button" onClick={() => setTrackedItems(timedIds, true)}
                          className="inline-flex items-center rounded-md border border-blue-300 bg-blue-50 px-2 py-1 text-[11px] font-bold text-blue-700 hover:bg-blue-100 dark:border-blue-800 dark:bg-blue-900/30 dark:text-blue-300 dark:hover:bg-blue-900/50 transition-colors whitespace-nowrap">
                          {i18n.pages.gathering_log.bookmark_alarm_track_all}
                        </button>
                        <button type="button" onClick={() => setTrackedItems(timedIds, false)}
                          className="inline-flex items-center rounded-md border border-slate-300 bg-white px-2 py-1 text-[11px] font-bold text-slate-600 hover:bg-slate-100 dark:border-slate-600 dark:bg-slate-800 dark:text-slate-300 dark:hover:bg-slate-700 transition-colors whitespace-nowrap">
                          {i18n.pages.gathering_log.bookmark_alarm_clear_all}
                        </button>
                      </>
                  ...
                  onCollapse={(_, collapsed) => setIsTimedUngroupedCollapsed(collapsed)}
                />
                {renderUngroupedGrid(timedUngroupedItems, isTimedUngroupedCollapsed, '__ungrouped_timed__')}
              </div>
            </div>
          )}
          ...
          {regularUngroupedItems.length > 0 && (
            <div className="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-slate-200 dark:border-slate-700 overflow-hidden">
              ...
              <div className="divide-y divide-slate-200 dark:divide-slate-600">
                <GroupHeader
                  group={{ id: '__ungrouped_regular__', name: i18n.pages.gathering_log.ungrouped, itemIds: regularUngroupedItems.map(i => i.id), collapsed: isRegularUngroupedCollapsed, createdAt: '', updatedAt: '' }}
                  itemCount={regularUngroupedItems.length}
                  onCollapse={(_, collapsed) => setIsRegularUngroupedCollapsed(collapsed)}
                />
                {renderUngroupedGrid(regularUngroupedItems, isRegularUngroupedCollapsed, '__ungrouped_regular__')}
              </div>
            </div>
          )}
        </div>
