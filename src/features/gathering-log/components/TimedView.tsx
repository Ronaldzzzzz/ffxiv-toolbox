...
export const TimedView: React.FC<TimedViewProps> = ({ data, currentType, completedItems, toggleComplete, hideCompleted }) => {
    ...
    const renderNodeCard = (node: NodeData & { statusInfo: NodeStatus }) => {
        ...
        return (
            <div
                ...
                style={isAnimating ? { animation: `pulse ${pulseDuration}s cubic-bezier(0.4, 0, 0.6, 1) infinite` } : {}}
            >
                <div className="p-3 flex-grow flex flex-col">
                    ...
                    <div className="flex items-start gap-3 mb-2">
                        ...
                        <img src={data.icons[itemId] ? `https://xivapi.com${data.icons[itemId]}` : 'https://xivapi.com/i/066000/066313_hr1.png'} className="w-10 h-10 object-contain rounded shrink-0 bg-slate-100 dark:bg-slate-700" />
                        <div className="flex-grow min-w-0">
                            <h4 className="font-bold text-sm text-slate-800 dark:text-slate-200 truncate leading-tight mb-1" title={getLocalizedText(item, lang)}>
    ...
    return (
        <div className="space-y-6 pb-20">
            ...
            {activeNodes.filter(n => !(hideCompleted && completedItems.has(n.items[0]))).length === 0 &&
                soonNodes.filter(n => !(hideCompleted && completedItems.has(n.items[0]))).length === 0 &&
                laterNodes.filter(n => !(hideCompleted && completedItems.has(n.items[0]))).length === 0 && (
